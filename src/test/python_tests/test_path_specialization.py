...
class CallbackObject:
    ...
    def check_for_argv_duplication(self, argv: Dict[str, str]):
        ...
        if argv["type"] == 4 and argv["message"].find("--format") >= 0:
            parts = argv["message"].split()
            count = len([x for x in parts if x.startswith("--format")])
            self.result = count > 1


def test_path():
    """Test linting using flake8 bin path set."""

    default_init = defaults.vscode_initialize_defaults()
    default_init["initializationOptions"]["settings"][0]["path"] = ["flake8"]
...
def test_workspace_settings_update_resolves_symlinks():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert stored_settings.get("args") == [
            "--ignore=E501"
